 (function (ko) {
  ...
  ko.components.register('sklik-table', {
    viewModel: TableViewModel,
    template:
    '<sklik-pager params="pager: pager, table: $data"></sklik-pager> \
    <table class="table table-striped" id="sklik-table"> \
      <thead class="sorting"> \
        <tr> \
          ...
          <!-- ko foreach: { data: visibleColumns, as: \'col\'} --> \
            <th> \
              <!-- ko if: col.sortable --> \
                <a data-bind="click: $parent.sortBy.bind($parent, col.id), \
                  text: col.name, \
                  css: { \
                    \'glyphicon-chevron-up\': col.id == $parent.order &amp;&amp; $parent.direction == \'SORT_ASC\', \
                    \'glyphicon-chevron-down\': col.id == $parent.order &amp;&amp; $parent.direction == \'SORT_DESC\' \
                  }" href="#" class="glyphicon"></a> \
              <!-- /ko --> \
              <!-- ko ifnot: col.sortable --> \
      ...
      <tfoot data-bind="foreach: sumRowsForTeplate"> \
